...
class TFcombine(nn.Module):
    def __init__(self):
        super(TFcombine, self).__init__()
        self.bn = nn.BatchNorm2d(22)
        self.t_conv1 = nn.Sequential(
            nn.Conv1d(22, 22, 3, padding=1),
            nn.ReLU()
    ...
    def forward(self,x):
        x = self.bn(x)
        B, C, F, T = x.shape
        x_avg = self.avgp(x)  # (b,c,1,1)
        a_t = torch.mean(x, dim=-2)  # (b,c,128)
        a_f = torch.mean(x, dim=-1)  # (b,c,360)
        a_t = self.t_conv1(a_t)
        a_t = self.t_conv2(a_t)
        a_t = a_t.unsqueeze(dim=-2)  # (b,c,1,128)
        f_avg = x_avg.repeat(1, 1, F, 1)
        a_f = self.f_conv1(a_f)
        a_f = self.f_conv2(a_f)
        a_f = a_f.unsqueeze(dim=-1)  # (b,c,360,1)
        t_avg = x_avg.repeat(1, 1, 1, T)
        a_tf = a_t * f_avg
        a_ft = a_f * t_avg
        attn = a_t * a_f  # (b,c,360,128)
        x_attn = (attn + a_tf + a_ft) * x
        return x_attn
...
class TFSE(nn.Module):
    def __init__(self):
        ...
        self.FC1 = nn.Sequential(nn.Linear(22,11),
            nn.ReLU()
        )
        self.FC2 = nn.Sequential(nn.Linear(11,22),
            nn.Sigmoid()
        )
        self.avgpool = nn.AdaptiveAvgPool2d((360, 128))
        self.conv = nn.Sequential(
            nn.Conv2d(22, 22, 3, padding=1),
            nn.ReLU()
        )
    ...
    def forward(self,x):
        ...
        a_tf = a_t * a_f#(b,c,360,128)
        x_attn = a_tf * x
        avg_x = self.avgpool(x_conv)
        avg_x = avg_x.permute(0, 2, 3, 1)
        squeeze_x = self.FC1(avg_x)
        excitation_x = self.FC2(squeeze_x)
        excitation_x = excitation_x.permute(0, 3, 1, 2)
        se_x = excitation_x * x_conv
        x_attn = x_attn + se_x
...
class MTAnet(nn.Module):
    def __init__(self, input_channel, drop_rate=0.1):
        ...
        self.out = nn.Sequential(
            # _DenseBlock(2, last_channel, 32, drop_rate),
            # Conv(32, 32, 1))
            TFatten())
            # TFcombine())
            # TFSE())
    ...
    def forward(self, input):
        # input_target = input
        # torch.save(input_target.to(torch.device('cpu')), "input1Tensor.pth")
        ...
        # detection = full_output
        # detection = nn.Softmax(dim=-2)(detection)
        # torch.save(detection.to(torch.device('cpu')), "fullband_Tensor.pth")
